...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate, int targetYear) {
        return presentValue * (Math.pow(((rate/100) + 1), CompoundInterest.numYears(targetYear)));
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return CompoundInterest.futureValue(presentValue, rate, targetYear)
                * Math.pow((1 - (inflationRate/100)), CompoundInterest.numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double savings = 0;
        for (int i = CompoundInterest.numYears(targetYear); i >= 0; i--) {
            savings += CompoundInterest.futureValue(perYear, rate, targetYear);
            targetYear--;
        }
        return savings;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate, double inflationRate) {
        return CompoundInterest.totalSavings(perYear, targetYear, rate)
                * Math.pow((1 - (inflationRate/100)), CompoundInterest.numYears(targetYear));
    ...
    static void printDollarFV(int targetYear, double returnRate,
        ...
        double nominalDollarValue = CompoundInterest.futureValue(1,returnRate, targetYear);
        double realDollarValue = CompoundInterest.futureValueReal(1,returnRate,targetYear,inflationRate);
