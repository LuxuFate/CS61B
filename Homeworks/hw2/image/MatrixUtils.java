 ...
 *
 *  @author Josh Hug and Matthew Lu
 */
...
public class MatrixUtils {
    ...
    /** Returns the value of e[r][c] if available.
     * @param e 2D matrix
     * @param r row
     * @param c column
     * */
    public static double get(double[][] e, int r, int c) {
        if ((r >= 0 && r <= e.length - 1) && (c >= 0 && c <= e[0].length - 1)) {
            return e[r][c];
        } else {
     ...
     * @return Returns the minimum value of these three doubles.
     * */
    ...
    /** Finds the minimum of three doubles.
     * @return Returns the 2D Array of the accumulate.
     * @param m a 2D Array
     * */
    public static double[][] accumulateVertical(double[][] m) {
        double[][] result = new double[m.length][m[0].length];
        System.arraycopy(m[0], 0, result[0], 0, result[0].length);
        for (int r = 1; r < m.length; r++) {
            for (int c = 0; c < m[0].length; c++) {
                result[r][c] = get(m, r, c)
                        + min3(get(result, r - 1, c - 1), get(result, r - 1, c),
                        get(result, r - 1, c + 1));
            }
        }
        return result;
     ...
     * @param orientation The orientation of how you want
     * @return Returns the 2D array of
     * the accumulate depending on the orientation
     * */
