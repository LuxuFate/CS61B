...
public class NFA {
    ...
    private class State {
        ...
        public Set<State> successors(char c) {
            if (!_edges.containsKey(c)) {
                ...
                Set<State> suc = new HashSet<State>();
                ...
                while (!fringe.isEmpty()) {
                    ...
                    if (seen.add(s)) {
                        suc.add(s);
                        Set<State> edge = s._edges.get(EPSILON);
    ...
    public boolean matches(String s) {
        Set<State> set = _startState.successors(EPSILON);
        set.add(_startState);
        for (int i = 0; i < s.length(); i++) {
            Set<State> snew = new HashSet<State>();
            for (State st: set) {
                snew.addAll(st.successors(s.charAt(i)));
            }
            set = snew;
            snew = new HashSet<State>();
            for (State st: set) {
                snew.addAll(st.successors(EPSILON));
            }
            set.addAll(snew);
        }
