...
public class TreeMap<K extends Comparable<K>, V> implements SimpleMap<K, V> {
    ...
    private TreeMapNode putHelper(TreeMapNode node, K key, V value) {
        ...
        if (node._left == null && key.compareTo(node._key) < 0) {
            node._left = new TreeMapNode(key, value, null, null);
        } else if (node._right == null && key.compareTo(node._key) > 0) {
            node._right = new TreeMapNode(key, value, null, null);
        } else {
            if (node._left != null && key.compareTo(node._key) < 0) {
                if (node._left._key.compareTo(key) == 0) {
                    node._left._value = value;
                } else {
                    node._left = putHelper(node._left, key, value);
                }
            } else if (node._right != null && key.compareTo(node._key) > 0) {
                if (node._right._key.compareTo(key) == 0) {
