...
public class ArrayHeap<T> {
    ...
    private void bubbleUp(int index) {
        while (index > 1 && index == min(index, getParentOf(index))) {
    ...
    private void bubbleDown(int index) {
        if (index < size()) {
            int minC = min(getLeftOf(index), getRightOf(index));
            int min = min(index, minC);
            if (index != min) {
                swap(index, min);
                bubbleDown(min);
            }
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i < size(); i++) {
            if (getNode(i).item().equals(item)) {
                double save = getNode(i).priority();
