...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        _allSquares = new ArrayList<Sq>(); _board = new Sq[_width][_height];
        _solnNumToPlace = new Place[last + 1]; int contains = last - 1;
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                int sqNum = _solution[i][j];
                if (sqNum == 1 || sqNum == last) {
                    Sq n = new Sq(i, j, sqNum, true, arrowDirection(i, j), 0);
                    _board[i][j] = n; _allSquares.add(n);
                    _solnNumToPlace[sqNum] = n.pl;
                } else {
                    Sq n = new Sq(i, j, 0, false, arrowDirection(i, j), -1);
                    _board[i][j] = n; _allSquares.add(n);
                    _solnNumToPlace[sqNum] = n.pl;
                }
            }
        }
        for (int ck = 0; ck < last; ck++) {
            for (int i = 0; i < _width; i++) {
                for (int j = 0; j < _height; j++) {
                    if (_solution[i][j] == ck) {
                        contains--;
        ...
        for (int each = 0; each < last; each++) {
            ...
            for (int i = 0; i < _width; i++) {
                for (int j = 0; j < _height; j++) {
                    if (_allSquares.get(each).connectable(_board[i][j])) {
                        successors.add(_board[i][j].pl);
                    }
                }
            }
            _allSquares.get(each)._successors = successors;
        }
        for (int each = 0; each < last; each++) {
            PlaceList predecessors = new PlaceList();
            for (int i = 0; i < _width; i++) {
                for (int j = 0; j < _height; j++) {
                    if (_board[i][j].connectable(_allSquares.get(each))) {
                        predecessors.add(_board[i][j].pl);
                    }
                }
            }
            _allSquares.get(each)._predecessors = predecessors;
        }
        _unconnected = last - 1;
    }
    ...
    boolean autoconnect() {
        ...
        for (Sq each: _allSquares) {
            for (Sq next: _allSquares) {
                if (each._sequenceNum != 0 && next._sequenceNum != 0) {
    ...
    void solve() {
        for (Sq temp: _allSquares) {
            for (int i = 0; i < _width; i++) {
                for (int j = 0; j < _height; j++) {
                    if (temp.x == i && temp.y == j) {
                        temp._sequenceNum = _solution[i][j];
                    }
                }
            }
        }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            if (Place.dirOf(this.x, this.y, s1.x, s1.y)
                    == this._dir && this != s1) {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0) {
                for (Sq temp = this; temp._successor
                        != null; temp = temp._successor) {
                    temp._successor._sequenceNum = temp.sequenceNum() + 1;
                }
            }
            if (s1.sequenceNum() != 0) {
                for (Sq temp = s1; temp._predecessor
                        != null; temp = temp._predecessor) {
                    temp._predecessor._sequenceNum = temp.sequenceNum() - 1;
                }
            }
            for (Sq temp = this._successor; temp
                    != null; temp = temp._successor) {
                temp._head = this._head;
            }
            if ((tgroup != 0 && sgroup == 0) || (tgroup == 0 && sgroup != 0)) {
                if (sgroup != 0) {
                    releaseGroup(sgroup);
                } else {
                    releaseGroup(tgroup);
        ...
        void disconnect() {
            ...
            int tgroup = this._group; int ngroup = next._group;
            if (_sequenceNum == 0) {
                if ((next._successor == null) && (_predecessor == null)) {
                    ...
                    _group = next._group = -1;
                        ...
                        ^ (next._successor == null && _predecessor != null)) {
                    if (this._predecessor == null) {
                        this._group = -1;
                        for (Sq temp = next; temp != null;
                    ...
                    this._sequenceNum = 0; next._group = newGroup();
                }
            } else {
                boolean prevfixed = false; boolean nextfixed = false;
                for (Sq temp = this; temp != null; temp = temp._predecessor) {
                    if (temp._hasFixedNum) {
                        prevfixed = true; break;
                    }
                }
                for (Sq temp = next; temp != null; temp = temp._successor) {
                    if (temp._hasFixedNum) {
                        nextfixed = true; break;
                    }
                }
                if (!prevfixed) {
                    if (this._predecessor != null) {
                    ...
                    for (Sq temp = this; temp != null;
                        ...
                        temp._sequenceNum = 0; temp._group = tgroup;
                    }
                }
                if (!nextfixed) {
                    ...
                    for (Sq temp = next; temp != null; temp = temp._successor) {
                        temp._sequenceNum = 0; temp._group = ngroup;
                    }
                }
            }
            for (Sq temp = next; temp != null; temp = temp._successor) {
