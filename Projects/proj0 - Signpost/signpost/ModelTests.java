...
public class ModelTests {
    ...
    public void copyTest() {
        Model oldm = new Model(tr(TEST00));
        oldm.solve();
        Model newm = new Model(oldm);
        newm.solve();
        System.out.println(oldm);
        System.out.println(newm);
        for (int x = 0; x < oldm.width(); x += 1) {
            for (int y = 0; y < oldm.height(); y += 1) {
                Sq old = oldm.get(x, y);
                Sq neww = newm.get(x, y);
                assertFalse("Sq should not be the same instance", old == neww);
                assertTrue("Sq should be equivalent objects", old.equals(neww));
            }
        }
    ...
    public void solvedTest1() {
        ...
        assertFalse("Model not solved yet.", model2.solved());
        model2.solve();
        assertTrue("Model should be solved.", model2.solved());
        checkNumbers(tr(SOLN2), model2, asList(1, 20));
    }
    ...
    @Test
    /*
    * assertEquals(2, model.arrowDirection(0,0));
//        assertEquals(8, model.arrowDirection(1,0));
//        assertEquals(6, model.arrowDirection(1,1));
//        assertEquals(0, model.arrowDirection(0,1));
//
//        assertEquals(8, model2.arrowDirection(0,0));
//        assertEquals(2, model2.arrowDirection(0,3));
//        assertEquals(5, model2.arrowDirection(1,2));
//        assertEquals(6, model2.arrowDirection(3,3));*/
