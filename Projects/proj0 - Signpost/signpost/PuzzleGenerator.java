...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        int unique2 = 0;
        Sq unisq2 = null;
        for (Place check: start.successors()) {
            if (start.connectable(model.get(check))) {
                unique1++;
                unisq1 = model.get(check);
            }
        }
        for (Place check: start.successors()) {
            if (start.sequenceNum() != 0) {
                if (start.connectable(model.get(check))
                        && model.get(check).sequenceNum()
                        == start.sequenceNum() + 1) {
                    unique2++;
                    unisq2 = model.get(check);
                }
            }
        }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        int unique = 1;
        Sq unisq = null;
        for (Place check: end.predecessors()) {
            if (model.get(check).connectable(end)) {
                unique--;
                unisq = model.get(check);
            }
        }
        if (unique == 0) {
            return unisq;
