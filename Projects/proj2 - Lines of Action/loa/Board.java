...
class Board {
    ...
    void initialize(Piece[][] contents, Piece side) {
        ...
        for (int r = 0; r < contents.length; r++) {
            for (int c = 0; c < contents[r].length; c++) {
                _board[numPieces] = contents[r][c];
                _board2D[r][c] = contents[r][c];
                numPieces++;
            }
        }
        _turn = side;
    ...
    void copyFrom(Board board) {
        ...
        initialize(board._board2D, board._turn);
    ...
    void retract() {
        assert movesMade() > 0;
        if (_moves.size() > 1) {
            Move remove = _moves.remove(movesMade() - 1);
            set(remove.getFrom(), get(remove.getTo()), _turn.opposite());
            if (remove.isCapture()) {
                set(remove.getTo(), _turn.opposite());
            } else {
                set(remove.getTo(), EMP);
            }
            if (_moves.size() > 0) {
                remove = _moves.remove(movesMade() - 1);
                set(remove.getFrom(), get(remove.getTo()), _turn.opposite());
                if (remove.isCapture()) {
                    set(remove.getTo(), _turn.opposite());
                } else {
    ...
    boolean isLegal(Square from, Square to) {
        ...
        if (!exists(from.col(), from.row()) || !exists(to.col(), to.row())) {
            return false;
        } else if (!get(from).equals(_turn)
                || !from.isValidMove(to) || blocked(from, to)) {
            return false;
        } else {
            int linePieces = 1;
            int dir = from.direction(to);
            int opdir = to.direction(from);
            for (int i = 1; i <= BOARD_SIZE; i++) {
                Square check = from.moveDest(dir, i);
                if (check != null) {
                    if (!get(check).equals(EMP)) {
                        linePieces++;
                    }
                }
            }
            for (int i = 1; i <= BOARD_SIZE; i++) {
                Square check = from.moveDest(opdir, i);
                if (check != null) {
                    if (!get(check).equals(EMP)) {
                        linePieces++;
                    }
            ...
            if (from.distance(to) != linePieces) {
                return false;
            }
        }
        return true;
    ...
    List<Move> legalMoves() {
        ...
        for (Square from: ALL_SQUARES) {
            if (get(from) != EMP && get(from).equals(_turn)) {
                for (Square to: ALL_SQUARES) {
                    if (!(to == from) && isLegal(from, to)) {
    ...
    private boolean blocked(Square from, Square to) {
        ...
        Square land = from.moveDest(dir, distance);
        if (exists(land.row(), land.col()) && get(land).equals(_turn)) {
            return true;
        }
        for (int i = 1; i < distance; i++) {
            Square check = from.moveDest(dir, i);
            if (check != null) {
                if (exists(check.row(), check.col())
                        && get(check).equals(_turn.opposite())) {
                    return true;
                }
            }
        }
    ...
    private void computeRegions() {
        ...
        for (Square sq: ALL_SQUARES) {
            if (get(sq) == BP && !visited[sq.row()][sq.col()]) {
                _blackRegionSizes.add(numContig(sq, visited, BP));
            } else if (get(sq) == WP && !visited[sq.row()][sq.col()]) {
    ...
    /** Current contents of the board as a 2D array. */
    private final Piece[][] _board2D = new Piece[BOARD_SIZE][BOARD_SIZE];
