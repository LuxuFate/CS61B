...
class BoardWidget extends Pad {
    ...
    private void mouseReleased(Square s) {
        if (_acceptingMoves) {
            if (_pressed != s && _pressed != null) {
                update = Move.mv(_pressed, s);
                update(_board);
            }
            _pressed = null;
        }
    ...
    /** Saver for pressed. */
    private Square _pressed = null;

    /** Update Move. */
    private Move update = null;
    ...
    synchronized void update(Board board) {
        ...
        if (update != null && _acceptingMoves) {
            _commands.offer(update.getFrom().toString()
                    + "-" + update.getTo().toString());
        }
        repaint();
    }
